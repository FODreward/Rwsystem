  ...
  const renderSection = () => {
    switch (activeSection) {
      case "dashboardHome":
        return (
          <>
            {/* Dashboard Overview - Welcome and Stats */}
            <section className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {/* Welcome Back Card - spans full width on mobile, 2/3 on desktop */}
              <Card className="md:col-span-2 bg-gradient-to-br from-primary-500 to-primary-600 p-6 rounded-2xl shadow-lg text-white flex flex-col justify-between">
                <CardHeader className="p-0 mb-4">
                  <CardTitle className="text-3xl font-extrabold drop-shadow-md">
                    Welcome Back, Survey Enthusiast!
                  </CardTitle>
                </CardHeader>
                <CardContent className="p-0 text-lg text-white/90">
                  Your journey to exciting rewards continues!
                </CardContent>
              </Card>

              {/* Wallet Balance Card - spans 1/3 on desktop, full width on mobile */}
              <Card className="bg-card-background p-6 rounded-2xl shadow-lg flex flex-col items-center justify-center text-center">
                ...
                <CardContent className="p-0 text-sm text-text-secondary">Wallet Balance</CardContent>
              </Card>

              {/* Other Stats - arranged in a responsive grid below */}
              <div className="grid grid-cols-1 sm:grid-cols-3 md:col-span-3 gap-6">
                <Card className="bg-card-background p-5 rounded-2xl shadow-lg flex flex-col items-center justify-center text-center">
                  <ClipboardList className="h-8 w-8 text-primary mb-2" />
                  <CardTitle className="text-2xl font-bold text-primary mb-1">
                    {isLoadingStats ? "..." : dashboardStats?.completed_surveys || 0}
                  </CardTitle>
                  <CardContent className="p-0 text-xs text-text-secondary">Surveys Completed</CardContent>
                </Card>
                <Card className="bg-card-background p-5 rounded-2xl shadow-lg flex flex-col items-center justify-center text-center">
                  <ReceiptText className="h-8 w-8 text-primary mb-2" />
                  <CardTitle className="text-2xl font-bold text-primary mb-1">
                    {isLoadingStats ? "..." : dashboardStats?.pending_redemptions || 0}
                  </CardTitle>
                  <CardContent className="p-0 text-xs text-text-secondary">Pending Redemptions</CardContent>
                </Card>
                <Card className="bg-card-background p-5 rounded-2xl shadow-lg flex flex-col items-center justify-center text-center">
                  ...
                  <CardContent className="p-0 text-xs text-text-secondary">Total Earned</CardContent>
                </Card>
              </div>
            </section>
